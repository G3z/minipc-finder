...
export const toCatalog = (devices: Device[]): CatalogDevice[] => devices.map(({ id, model, brand, scores, cpu, gpu, memory, storage, ports, network, power, physical, support, seller, pricing, releaseDate, discontinued }) => ({ id, model, brand, scores, cpu, gpu, memory, storage, ports, network, power, physical, support, seller, pricing: { barebone: pricing.barebone, variants: pricing.variants }, minPrice: [pricing.barebone, ...pricing.variants.map(variant => variant.usd)].filter((price): price is number => price !== null && price > 0).sort((a, b) => a - b)[0] ?? null, releaseDate, discontinued }));
